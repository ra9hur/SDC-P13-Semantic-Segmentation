...
_CONV_DEFS = [
    ...
    DepthSepConv(kernel=[3, 3], stride=1, depth=256),   # 5 - layer_3
    DepthSepConv(kernel=[3, 3], stride=2, depth=512),
    DepthSepConv(kernel=[3, 3], stride=1, depth=512),
    DepthSepConv(kernel=[3, 3], stride=1, depth=512),   # 8 - layer_4
    DepthSepConv(kernel=[3, 3], stride=1, depth=512),
    DepthSepConv(kernel=[3, 3], stride=1, depth=512),
    DepthSepConv(kernel=[3, 3], stride=1, depth=512),   # 11 - layer_5
    DepthSepConv(kernel=[3, 3], stride=2, depth=1024),
    DepthSepConv(kernel=[3, 3], stride=1, depth=1024)   # 13 - layer_6
]
  ...
  with tf.variable_scope(scope, 'MobilenetV1', [inputs, num_classes],
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      with tf.variable_scope('Logits'):
        ...
        net = slim.avg_pool2d(net, kernel_size, padding='SAME', scope='AvgPool_1a')
        end_points['AvgPool_1a'] = net
        # 1 x 1 x 1024
        net = slim.dropout(net, keep_prob=dropout_keep_prob, scope='Dropout_1b')
        logits = slim.conv2d(net, num_classes, [1, 1], activation_fn=None, normalizer_fn=None, scope='Conv2d_1c_1x1')
        #logits = slim.conv2d(net, 4096, [1, 1], scope='fc7')
        if spatial_squeeze:
